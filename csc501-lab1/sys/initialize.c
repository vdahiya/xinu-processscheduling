 ...
 *
 * changelist:
 * 1) added header file rescheduler.h
 * 2) Declared new kernel variable epochlst
 * 3) added code to initialize epoch list in nulluser() method
 *
 * */
...
#include <io.h>
#include <stdio.h>
#include <rescheduler.h> 	// (1)added newly to support initialization of rescheduler specific data structures
...
#endif
struct epochentry epochlst[NPROC+2]; /* (1)epoch list.
 	 	 	 	 	 0 - (NPROC-1) for each process, NPROC-th entry for head, (NPROC+1)th entry for tail */
...
LOCAL int sysinit()
{
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	pptr->pprio = 0;
	currpid = NULLPROC;
	pptr->counter = pptr->pprio;	/* (1) set the counter to its priority - used only in linux-like scheduler */
	...
	/* (1) initialize epoch list - used only for linux-like scheduling */
	epochlst[NPROC].pprio = MININT;
	epochlst[NPROC].enext = 0;	/* points to next entry in epoch list or tail(if list is empty)  */
	epochlst[0].pprio = proctab[0].pprio;
	epochlst[0].enext = NPROC + 1;
	epochlst[NPROC+1].pprio = MAXINT;
	epochlst[NPROC+1].enext = -1;	/* enext value of -1 indicates its the tail entry  */
	rolloverthreshold = 1;
